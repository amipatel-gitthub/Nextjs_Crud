...
export default function page() {
    ...
    const handleSubmit = (e) => {
        ...
        localStorage.setItem("User_Form", JSON.stringify(records))
        if (isEditing) {
    ...
    const createRecord = (data) => {
        ...
        localStorage.setItem("User_Form", JSON.stringify([...records, newRecord]))
    };
    ...
    const updateRecord = (data) => {
        ...
        setRecords(up_data);
        localStorage.setItem("User_Form", JSON.stringify(up_data))
    };
    ...
    useEffect(() => {
        const storedData = JSON.parse(localStorage.getItem("User_Form")) || []
    ...
    const [itemParPage, setItemPerPage] = useState(2)

    const [totalItems] = useState(100)
    const totalPages = Math.ceil(totalItems / itemParPage)
    ...
    const handleDropdown = () => {
        return (
            <select value={itemParPage} onChange={handlePageRender}>
    ...
    const handleSearch = (event) => {
        ...
        if (value === "") {
            setRecords(JSON.parse(localStorage.getItem("User_Form")) || [])
        }
        else {
    ...
    const noRecordsFound = (
        <div>
            {records.length === 0 && searchItem !== " " && (
                <p className="text-red-600 ms-2 mt-2 font-mono font-thin">No result found in '{searchItem}'</p>
            )}
        </div>
    )
