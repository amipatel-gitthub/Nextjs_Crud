...
export default function page() {
  ...
  useEffect(() => {
    const storeRecords = JSON.parse(localStorage.getItem("Next_Crud")) || []
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("Next_Crud", JSON.stringify(records));
    sessionStorage.setItem("Next_Crud", JSON.stringify(records));
    Cookies.set("next_Crud", JSON.stringify(records));
  ...
  const updateRecord = (record) => {
    const up_data = records.map((value) =>
      value.id === record.id ? { ...value, ...record } : value
    );
    setRecords(up_data);
  };
  ...
  const displayItem = records.slice(startIndex, endIndex);
  ...
  const handlePageRendering = (event) => {
  ...
  const handleDropDown = () => {
    return (
      <select value={itemParPage} onChange={handlePageRendering}>
  ...
  const handleSearch = (event) => {
    ...
    if (value === "") {
      setRecords(JSON.parse(localStorage.getItem("Next_Crud")) || []);
    } else {
